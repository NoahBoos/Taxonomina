...
<div class="space-y-4">
    ...
    <div class="space-y-2">
        ...
        <div class="flex flex-row gap-2">
            <Select name="selected-theme" label="Thème" options={ THEMES } bind:value={ theme } onChange={ () => { updateSetting("selectedTheme", theme) }} />
            <Select name="theme-variant" label="Variante" options={ THEME_VARIANTS } bind:value={ theme_variant } onChange={ () => { updateSetting("themeVariant", theme_variant) }} />
        </div>
    </div>
    <div class="space-y-2">
        <h3>Accessibilité</h3>
        <div class="flex flex-row gap-2">
            <Select name="font-size" label="Taille du texte" options={ FONT_SIZES } bind:value={ font_size } onChange={ () => { updateSetting("fontSize", font_size) }} />
            <Select name="elements-per-page" label="Nombre d'éléments à afficher dans les navigateurs" options={ ELEMENTS_PER_PAGE } bind:value={ elements_per_page } onChange={ () => { updateSetting("elementsPerPage", Number(elements_per_page)) }} />
        </div>
        <div class="flex flex-row gap-2">
